...
describe("genererBilanDocx", () => {
  it("produit un buffer .docx valide (signature ZIP)", async () => {
    const buffer = await genererBilanDocx(validBilanFixture);
...
describe("nomFichierBilanDocx", () => {
  it("respecte le nommage imposé (§7)", () => {
    expect(nomFichierBilanDocx("Alice Dupont", "2024-01-01", "2024-03-31")).toBe(
      "Bilan_trimestriel_Alice_Dupont_2024_01_01_2024_03_31.docx"
    );
  });

  it("retire les accents pour un nom de fichier sûr", () => {
    expect(nomFichierBilanDocx("Léo Bénéficiaire", "2024-01-01", "2024-03-31")).toBe(
